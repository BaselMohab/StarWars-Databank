    ...
    <div className="p-8 mt-32 bg-black min-h-screen">
      <div className="flex justify-end mb-8">
        <Breadcrumbs className="text-yellow-500 bg-transparent">
          ...
          <Link to="/planets" className="text-xl text-yellow-400 hover:text-white capitalize">
            planets
          </Link>
        </Breadcrumbs>
      </div>
      <div className="grid gap-8 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        {planets.map(planet => {
          ...
          const imageUrl = starWarsData.planets[planet.name]
          return (
            <Card key={id} className="bg-gray-900 border border-yellow-500 shadow-xl rounded-lg overflow-hidden">
              <div className="relative h-60">
                <img src={imageUrl} alt={planet.name} className="w-full h-full object-cover" />
              </div>

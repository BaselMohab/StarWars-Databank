...
export default function PersonSection({title, items, singleItem = false}) {
    return (
        <div className="mb-20">
            ...
            {items.length > 0 ? (
                    ...
                    {items.map((item, index) => (
                        <Card className="w-full bg-transparent border border-gray-600 shadow-lg card" key={index}>
                            <CardHeader floated={false} className="h-80">
                                <img src={item.imageUrl} alt={item.name || item.title} />
                            </CardHeader>
                ...
                <p className="text-gray-400 italic">No {title} available for this person.</p>
            )}
        </div>
    );
}
